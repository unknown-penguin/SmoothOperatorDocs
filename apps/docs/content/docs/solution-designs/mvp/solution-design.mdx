...
Deliver a production-capable MVP for live F1 telemetry with minimal operational overhead and fastest time-to-market. Supports 1–2 cars at ~200 Hz with near-real-time dashboards, basic alerts, and historical queries.
...
- [x] Ingestion via MQTT → Redpanda (Kafka API compatible)
...
- [x] Support for 1–2 cars at 200 Hz telemetry rate
...
- 1000-record DB batches provide optimal write throughput (≈12k inserts/sec; CPU ~45% at peak)
...
- p95 processing latency: 280–450 ms (1 car @ 200 Hz)
...
- Data volume per car per race (~2h): ~1.4M records; ~2.8 GB uncompressed; ~400 MB compressed (season ≈18 GB)
...
- [ ] WebSocket reconnect with exp backoff (1s → 30s)
...
- [ ] Load tests for 1–2 cars @ 200 Hz
...
1. Foundation (Weeks 1–2): infra, CI/CD, monitoring baseline
2. Core Services (Weeks 3–4): MQTT ingestion, Redpanda, processor, Timescale schema
3. API & Realtime (Weeks 5–6): REST queries, Centrifugo, WS endpoint, auth
4. Frontend (Weeks 7–8): dashboards, live charts, history, alert UI
5. Testing & Hardening (Weeks 9–10): perf, security, runbooks, pre-prod
...
- Business: 10+ active engineers; &lt;0.1% data loss; ≤$2k/month; insights &lt; 2s
